...
def test_epoch(train_loader, model, optimizer, exp_path, epoch):
    model.train()
    lossavg = 0
...
def test(model, exp_path, dataloader):
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(dataloader):
            ...
            pred = model(images)
            preds += [(box, label, score) for box, label, score in pred['boxes'], pred['labels'], pred['scores']]
...
if __name__ == '__main__':
    ...
    test(dataset_.train_data_loader, model, optimizer, exp_path, epoch)
